...
public class SerialSocket extends BluetoothGattCallback {
    ...
    // Memory leak wont occur
    public static SerialSocket single_instance = null;
    ...
    public static SerialSocket getInstance() {
        if (single_instance == null) {
            single_instance = new SerialSocket();
    ...
    public void updateListener(SerialListener listener) {
        if (listener != null) {
    ...
    private void onPairingBroadcastReceive(Context context, Intent intent) {
        ...
        switch (intent.getAction()) {
            case BluetoothDevice.ACTION_PAIRING_REQUEST:
                ...
                onSerialConnectError(new IOException("Pairing request"));
    ...
    public void onMtuChanged(BluetoothGatt gatt, int mtu, int status) {
        ...
        Log.e(TAG, "onMtuChanged: " + mtu + "|" + status);
        connected = true;
        onSerialConnect();
    }
     ...
     * write
     */
    public void write(SerialListener listener, byte[] data) throws IOException {
        this.listener = listener;
        ...
        synchronized (writeBuffer) {
            ...
            // (WT-IOT is using ESP-BLE) In ESP-BLE we can send more than 20Bytes
            data0 = data;
            ...
            // so, we don't want 2D ByteArr for writeCharacteristic
        }
        if (data0 != null) {
